...
- [x] **Step 1: Replace `src/main.cpp` with the safe-init skeleton**
...
- [x] **Step 2: Create a local (untracked) `include/secrets.h` from the example so the build can link**

Run: `cp include/secrets.h.example include/secrets.h`
(This file is gitignored — never staged.)

- [x] **Step 3: Verify the esp8266 environment compiles**

Run: `pio run -e esp8266`
Expected: `SUCCESS`

- [x] **Step 4: Commit** (secrets.h itself must NOT be staged — verify with `git status` that only `src/main.cpp` is listed)
...
**Статус: выполнено.** Коммит `6985f83`. esp8266 собирается (`SUCCESS`, RAM 34.5%/
Flash 25.7%). Локальный `include/secrets.h` создан из примера, не закоммичен
(`git status` подтверждает — staged только `src/main.cpp`).
...
- [x] **Step 1: Add sensor bus objects and cached readings** (insert after the `#include` block in `src/main.cpp`)
...
- [x] **Step 2: Add the fast-poll (5s) and radiator-poll (60s) tick functions** (insert before `setup()`)
...
- [x] **Step 3: Wire async mode and initial requests into `setup()`, and the ticks into `loop()`**
...
**Статус: выполнено.** Коммит `e8e0ab8`. esp8266 собирается (`SUCCESS`, RAM 34.7%/
...
- [x] **Step 1: Add MQTT includes, client instance, and reconnect timer** (insert alongside other globals)
...
- [x] **Step 2: Add the command callback and non-blocking reconnect function** (insert before `setup()`)
...
- [x] **Step 3: Wire MQTT server/callback setup into `setup()`, and `loop()`/reconnect into `loop()`**
...
**Статус: выполнено.** Коммит `e692a61`. esp8266 собирается (`SUCCESS`, RAM 35.3%/
Flash 27.3%), native-тесты не затронуты (37/37 PASSED). API `connect()`/
`setCallback()` PubSubClient сверен через context7 — совпадает с планом.
